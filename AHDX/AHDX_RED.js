...
function getuser(phone,num) {
    return new Promise(resolve =>
        request(taskUrl('getuser', {"queryDate": formatTime(new Date()), "phone": `${phone}`}), async (error, response, body) => {
            if (!error && response.statusCode === 200) {
                let data = JSON.parse(Decrypt(body))
                if (data.code === 0) {
                    let phoneNum = data.data.phone;
                    console.log("开始拆红包")
                    await getTop100GiveRed(phone)
                    console.log("\n我的红包：")
                    await getMyReceiveRed(phone,phoneNum)
                } else {
...
function getTop100GiveRed(phone) {
    return new Promise(resolve => {
        request(taskUrl('getTop100GiveRed', {"queryDate": formatTime(new Date()), "refresh": `1`}), async (error, response, body) => {
            if (!error && response.statusCode === 200) {
                let data = JSON.parse(Decrypt(body))
                if (data.code === 0) {
                    let list = data.data;
                    for (let i = 0; i < list.length; i++) {
                        let seqid = list[i].seqid;
                        await receiveRed(phone, seqid)
                        await $.wait(5000)
                        if (isReceiveRed) {
                            break;
                        }
...
async function receiveRed(phone,giveseqid) {
    return new Promise(resolve => {
        request(taskUrl('receiveRed', {"queryDate": formatTime(new Date()), "giveseqid": `${giveseqid}`, "phone": `${phone}`}), async (error, response, body) => {
            if (!error && response.statusCode === 200) {
                let data = JSON.parse(Decrypt(body))
                if (data.code === 0) {
                    $.log(data.msg)
...
function getMyReceiveRed(phone,phoneNum) {
    return new Promise(resolve => {
        request(taskUrl('getMyReceiveRed', {"queryDate": formatTime(new Date()), "type": `0`, "phone": `${phone}`}), async (error, response, body) => {
            if (!error && response.statusCode === 200) {
                let data = JSON.parse(Decrypt(body))
                if (data.code === 0) {
                    let list = data.data.user;
                    for (let i = 0; i < list.length; i++) {
                        let company  = list[i].company;
                        let awardname  = list[i].awardname;
                        let requirement  = list[i].requirement;
                        let seqid  = list[i].seqid;
                        if (list[i].requirement == null) {
                            requirement = "";
                        }
                        console.log(awardname+company+" "+requirement)
                        body1 += awardname+company+" ";
                        console.log("领取红包")
                        await $.wait(5000)
...
function receiveRedPackage(phone,receiveid) {
    return new Promise(resolve => {
        request(taskUrl('receiveRedPackage', {"queryDate": formatTime(new Date()), "receiveid": `${receiveid}`, "phone": `${phone}`}), async (error, response, body) => {
            if (!error && response.statusCode === 200) {
                let data = JSON.parse(Decrypt(body))
