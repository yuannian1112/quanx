...
Surge
[Script]
小米运动 = type=cron,cronexp="15 17 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/xzxxn777/quanx/main/XmSport/XmSport.js
QuanX
[task_local]
# 小米运动
15 17 * * * https://raw.githubusercontent.com/xzxxn777/quanx/main/XmSport/XmSport.js, tag=小米运动, img-url=https://raw.githubusercontent.com/58xinian/icon/master/xmyd.png, enabled=true
Loon
[Script]
cron "15 17 * * *" script-path=https://raw.githubusercontent.com/xzxxn777/quanx/main/XmSport/XmSport.js, tag=小米运动
*/

const $ = new Env('小米运动-账号密码版');
...
let phone = ($.isNode() ? process.env.phone : $.getdata("phone")) || "";
let pwd = ($.isNode() ? process.env.pwd : $.getdata("pwd")) || "";
let app_token = '';
let code = '';
let userid = '';
...
async function login() {
    return new Promise(resolve => {
        const options = {
            ...
            body : `client_id=HuaMi&country_code=CN&password=${pwd}&redirect_uri=https%3A//s3-us-west-2.amazonaws.com/hm-registration/successsignin.html&state=REDIRECTION&t=${Date.now()}&token=access&token=refresh`

        }
        $.post(options, async (err, req, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
                    if(req.options.url.searchParams) {
                        if(req.options.url.searchParams.has("access")) {
                            code = req.options.url.searchParams.get("access")
                            await getUserInfo();
                        } else {
                            console.log(`账号没有获取到登录code`)
                        }
                    } else {
                        console.log(`账号获取登录code失败`)
                    }
                }
            } catch (e) {
                $.logErr(e, resp)
...
function change_step() {
    ...
    return new Promise(resolve => {
        const options = {
            ...
            "body": `userid=${userid.toString()}&last_sync_data_time=1597306380&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json=${dataJSON}`,
            "headers": {
        ...
        $.post(options, (err, resp, data) => {
            try {
                if (err) {
                    ...
                    $.changeStepRes = JSON.parse(data.body);
                    console.log(`修改步数结果:${$.changeStepRes.message}`);
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
